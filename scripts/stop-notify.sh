...
case "$app" in
  Terminal|iTerm2|Code|Cursor|Comet)
    (sleep 30 && terminal-notifier -title 'You there?' -message 'Claude finished your task.' && afplay /System/Library/Sounds/Bottle.aiff) &
    echo $! > "$PIDFILE"
    exit 0
    ;;
esac
...
terminal-notifier -title 'Done!' -message 'Check your Claude terminal.' && afplay /System/Library/Sounds/Purr.aiff &
exit 0
